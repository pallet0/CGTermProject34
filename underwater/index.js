...
// Geometries
const waterGeometry = new THREE.PlaneBufferGeometry(4, 4, waterSize, waterSize);
...
textureLoader.load('ocean2.png', function(tileTexture) {
  tileTexture.wrapS = tileTexture.wrapT = THREE.RepeatWrapping;
  tileTexture.repeat.set(1, 1); // 원하는 만큼 반복
  ...
  //   new THREE.PlaneGeometry(tankSize, tankSize),
  //   wallMaterial
  // );
  // bottom.position.set(0,  0,0.01); // z=0에 위치
  ...
  const wall1 = new THREE.Mesh(
    new THREE.PlaneGeometry(tankSize,tankHeight),
    wallMaterial
  );
  ...
  const wall3 = new THREE.Mesh(
    new THREE.PlaneGeometry(tankHeight,tankSize),
    waterMaterial
  );
...
class WaterSimulation {
  ...
  // Add a drop of water at the (x, y) coordinate (in the range [-2, 2])
...
class Caustics {
  ...
  constructor() {
    ...
    this._waterGeometry = new THREE.PlaneBufferGeometry(4, 4, waterSize, waterSize);
